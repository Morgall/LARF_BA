...
from rolling_lookahead_dt_gurobi.oct.optimal_tree_gurobi import generate_model_gurobi, train_model_gurobi, predict_model_gurobi

from rolling_lookahead_dt_gurobi.oct.tree import *


def rolling_optimize_gurobi(predefined_model: Optional[dict],
    ...
    y_idx = 0
    predefined_model.pop('model') #necessary otherwise deepcopy of predefined_model fails
    predefined_model.pop('params') #necessary otherwise deepcopy of predefined_model fails
    final_model = copy.deepcopy(predefined_model)
    #final_model = predefined_model.copy()
    ...
    for level in range(1, target_depth - main_depth + 1): #for each level of the rolling tree, "breadth first search approach"
        ...
        for node_ in parents_to_optimize:
            ...
            main_model = generate_model_gurobi(P=features,
                                        K=sub_K[node_],
                                        ...
                                        criterion=criterion)
            main_model = train_model_gurobi(model_dict=main_model,
                                     data=new_train_data_dict[node_],
                                     P=features)
            result_ = predict_model_gurobi(
                data=new_train_data_dict[node_],
        ...
        ####### hier ist drin wie die train daten im letzten Teilbaum performen; dict in predict_model_gurobi)
        
        final_model["depth"] = main_depth + level
        result_training_data = predict_model_gurobi(data=train_data,
        ...
        ####### hier ist drin wie die test daten im letzten Teilbaum performen; dict in predict_model_gurobi)
        result_test_data = predict_model_gurobi(data=test_data,
                               model_dict=final_model,
                               P=features,
                               pruned_nodes=pruned_nodes)
